...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('application/assets/plugins/fineuploader/jquery.fineuploader-5.0.5.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadAjaxContent(url){
	$('.preloader').show();	
	//url = 'ajax/forms_file_uploader_01.html';
	//alert(url);
	$.ajax({
		...
		success: function(data) {
			...
			//$('#ajax-content').load("ajax/file_uploader.php");  //ajax預設載入改寫 (重要!!)
			$('#ajax-content').load("ajax/tables_datatables_01.php");  //ajax預設載入改寫 (重要!!)
			$('.preloader').hide();
		},
...
---------------------------------------------*/
function FileUpload(){	
	var manualuploader = $('#bootstrapped-fine-uploader').fineUploader({
		...
		request: {			
			...
			//endpoint: 'localhost/aaa.html'
			//
			endpoint:'aaa.php'
			//endpoint:'aaa.php'					
			//endpoint: 'D:\xampp\htdocs\file_manager\file_manager2\aa'
		},
		validation: {
			allowedExtensions: ['jpeg', 'jpg', 'gif', 'png', 'doc', 'pdf' , 'xlsx' , 'pptx']
		}
	});		
}
