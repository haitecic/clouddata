...
{    
    ...
    {	
		...
		$columns = array();
		//將列表所要的欄位記錄在session中
		$this->session->set_userdata('first_item', $obj['column0']);
		$this->session->set_userdata('second_item', $obj['column1']);
		$this->session->set_userdata('third_item', $obj['column2']);
		$this->session->set_userdata('fourth_item', $obj['column3']);
		...
		$json_data = $this->project_model->get_datatable_record($parameter, $DB_table, $columns);
		echo $json_data;
